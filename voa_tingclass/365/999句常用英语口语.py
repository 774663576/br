...
def extract_lesson_info(li_tag):
    ...
    try:
        ...
        # match = re.match(r'高频英语口语对话600段[:：](\d+)\s+(.+)', text)
        # match = re.match(r'大忙人学社交(?:英语)?口语\s*(?:第)?(\d+)(?:期)?\s*[：:]?\s*(.+)', text)
        match = 1
...
def scrape_lessons(base_url):
    ...
    if all_lessons:
        ...
        with open('999句常用英语口语.json', 'w', encoding='utf-8') as f:
...
def main():
    base_url = 'https://www.tingclass.net/list-6087'
