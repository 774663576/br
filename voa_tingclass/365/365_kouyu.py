...
def extract_lesson_info(li_tag):
    ...
    try:
        ...
        # match = re.match(r'365天英语口语大全(\d+)\.(.+)', text)
        match = re.match(r'365天英语口语之出国旅游口语.*? 第(\d+)期:([^\s]+)', text)

        if match:
            index, title = match.groups()
            # 构造新的URL格式
            new_url = f'http://readingstuday.top/kouyu/365/{CATEGORY}/{index}.html'
            return {
                'url': new_url,
                'index': index,
                'title': title,
...
def save_lesson_content(url, lesson_info):
    ...
    try:
        ...
        if not content_div:
            content_div=soup.find('div',{'id':'fenlei'})
        if content_div:
            # 使用index作为文件名
            html_path = os.path.join('365_kouyu', f'{lesson_info["index"]}.html')
...
def scrape_lessons(base_url):
    ...
    if all_lessons:
        ...
        with open('365_kouyu/all_lessons.json', 'w', encoding='utf-8') as f:
...
def main():
    base_url = 'https://www.tingclass.net/list-10305'
